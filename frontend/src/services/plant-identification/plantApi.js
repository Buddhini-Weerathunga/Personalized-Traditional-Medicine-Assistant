...
/**
 * Get AI-generated plant description via Groq for any plant name
 * @param {string} plantName - The name of the plant to look up
 * @returns {Promise} - Structured plant data generated by Groq
 */
export const getGroqPlantDescription = async (plantName) => {
  try {
    const response = await axios.post(`${PLANT_API_BASE}/groq-description`, { plantName });
    return response.data;
  } catch (error) {
    console.error('Error fetching Groq plant description:', error);
    throw new Error(error.response?.data?.message || 'Failed to get AI plant description.');
  }
};
