...
const PlantDescriptionDetail = () => {
  ...
  const displayResult = plantDetails
    ? { ...plantDetails, confidence: result?.confidence || 0, scientificName: plantDetails.scientificName || result?.scientificName }
    : result;
  const displayImage = image || null;
  const isAIGenerated = displayResult?.aiGenerated || false;

  // Auto-fetch an image from Unsplash for AI-generated plants with no image
  const [aiPlantImage, setAiPlantImage] = useState(null);
  useEffect(() => {
    if (isAIGenerated && !displayImage && displayResult?.plantName) {
      const query = encodeURIComponent(displayResult.plantName + ' medicinal plant');
      // Use Unsplash source (free, no API key needed)
      setAiPlantImage(`https://source.unsplash.com/800x500/?${query}`);
    }
  }, [isAIGenerated, displayImage, displayResult?.plantName]);

  const finalImage = displayImage || aiPlantImage;
            ...
            <div className="lg:col-span-2 flex flex-col gap-6">
              ...
              {finalImage ? (
                <div className="bg-white rounded-2xl p-3 shadow-sm border border-gray-100">
                  <img src={finalImage} alt={displayResult.plantName} className="w-full h-auto max-h-[400px] object-cover rounded-xl" />
                  {isAIGenerated && aiPlantImage && !displayImage && (
                    <p className="text-center text-[10px] text-gray-400 mt-2">Image from Unsplash</p>
                  )}
                </div>
              ) : (
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center justify-center" style={{ minHeight: 120 }}>
                  <svg className="w-14 h-14 text-emerald-200" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}><path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z" /></svg>
                </div>
              )}
