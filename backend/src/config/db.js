...
const connectDB = async () => {
  try {
    // Remove useNewUrlParser and useUnifiedTopology as they're deprecated
    const conn = await mongoose.connect(process.env.MONGO_URI || "mongodb+srv://your-username:your-password@ac-xfgstr1-shard-00-00.issewk8.mongodb.net/yoga-therapy?retryWrites=true&w=majority");
    
    console.log(`✅ MongoDB Connected: ${conn.connection.host}`);
    
    // Initialize default poses if they don't exist
...
async function initializePoses() {
  ...
  const defaultPoses = [
    ...
    },
    {
      ...
      benefits: ["Strengthens legs and ankles", "Stretches hips and chest", "Builds stamina"],
  ...
  for (const pose of defaultPoses) {
    const exists = await YogaPose.findOne({ name: pose.name });
    if (!exists) {
      await YogaPose.create(pose);
      console.log(`✅ Added default pose: ${pose.name}`);
    }
  }
}
