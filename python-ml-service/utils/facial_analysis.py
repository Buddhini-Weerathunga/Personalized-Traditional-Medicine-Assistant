# python-ml-service/utils/facial_analysis.py
...
# Body Outline SVG for visualization (if needed later)
...
# Age-specific average ratios (from your research)
AGE_SPECIFIC_RATIOS = {
    "children": {  # 6-15 years
...
def analyze_with_mediapipe(image_data: bytes):
...
def analyze_skin_texture(skin_img: bytes):
...
def calculate_vata_score(face_results, age_group="adults"):
    ...
    if "face_ratio" in face_results:
        fr = face_results["face_ratio"]
        if fr < 0.85:
            score += 15

    if "face_width" in face_results and "face_height" in face_results:
        ratio = face_results["face_height"] / face_results["face_width"]
        if ratio > 1.5:
            score += 10
...
def get_dosha_description(dosha: str) -> str:
    descriptions = {
...
def get_dosha_recommendations(dosha: str):
    recommendations = {
        "vata": [
            "Follow a regular daily routine with consistent meal and sleep times",
...
def analyze_facial_features(face_img: bytes, skin_img: bytes = None):
    """Full pipeline: MediaPipe + texture + dosha scoring."""
    ...
    # OPTIONAL: You can add DeepFace age detection here if you want (like in your original)

    skin_features = analyze_skin_texture(skin_img or face_img)
...
def analyze_image_rule_based(image_bytes: bytes) -> dict:
    """
    Entry function used by FastAPI.
    Currently uses one image for both face + skin analysis.
    """
    return analyze_facial_features(image_bytes, image_bytes)
